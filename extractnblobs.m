...
if nargin == 15
    ...
    for k = 1:length(ax1)
        ...
        % Define o raio como sendo metade da medida do menor lado do ret�ngulo
        radius(k) = min((ax2(k) - ax1(k)), (ay2(k) - ay1(k))) / 2;

        % Marca a regi�o na imagem bin�ria
        labeled(round(ay1(k)):round(ay2(k)), round(ax1(k)):round(ax2(k))) = 1;
    end

    ndetect = length(ax1);  % N�mero de blobs detectados
    foremm = labeled;  % A imagem processada � simplesmente a imagem bin�ria gerada
